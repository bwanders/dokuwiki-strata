...
class syntax_plugin_strata_select extends DokuWiki_Syntax_Plugin {
    ...
    /**
     * This method renders the container div for any strata select.
     *
     * The div tag will contain all give classes plus additional metadata, e.g., generated by the ui group
     * @param R the renderer
     * @param data the custom data from the handle phase
     * @param additionalClasses array containing classes to be set on the generated div
     */
    function strata_div_open(&$R, $data, $additionalClasses) {
        $p = $data['strata-ui'];
        $d = array();

        $showGlobalFilter = $p['global filter'][0] == 'y';
        $showFilter = trim(implode($p['filter']), 'n') != '';
        $showLimit = isset($p['limit']);
        $showInfo = isset($p['info']) ? $p['info'][0] == 'y' : $showLimit; // By default, show filter when we limit the results

        if ($showGlobalFilter) $d['strata-ui-global-filter'] = null;
        if ($showFilter) $d['strata-ui-filter'] = $p['filter'];
        if ($showLimit) $d['strata-ui-limit'] = array_map('intval', $p['limit']);
        if ($showInfo) $d['strata-ui-info'] = null;
        $d['strata-ui-sort'] = $p['sort'];

        $classes = implode(' ', array_merge($additionalClasses, array_keys($d)));
