...
class syntax_plugin_stratabasic_entry extends DokuWiki_Syntax_Plugin {
    ...
    function handle($match, $state, $pos, &$handler) {
        ...
        foreach(preg_split('/\s+/',trim($header[1])) as $class) {
            $result['triples'][] = array('key'=>'class','value'=>$class,'type'=>null);
        }
        ...
        foreach($lines as $line) {
            ...
            if(preg_match('/^([-a-zA-Z0-9 ]+)(?:_([a-z0-9]+))?(\*)?:(.*)$/',$line,$parts)) {
                if($parts[3] == '*') {
                    $values = array_map('trim',explode(',',$parts[4]));
                } else {
                    $values = array(trim($parts[4]));
                }
                foreach($values as $v) {
                    $result['triples'][] = array('key'=>$parts[1],'value'=>$v,'type'=>$parts[2]);
                }
            } else {
